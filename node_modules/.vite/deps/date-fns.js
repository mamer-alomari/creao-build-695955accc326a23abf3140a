...
*/
function clamp(date, interval$1) {
	return min([max([date, interval$1.start]), interval$1.end]);
}
...
function eachDayOfInterval(interval$1, options) {
	const startDate = toDate(interval$1.start);
	const endDate = toDate(interval$1.end);
...
function eachMinuteOfInterval(interval$1, options) {
	const startDate = startOfMinute(toDate(interval$1.start));
	const endDate = toDate(interval$1.end);
	let reversed = +startDate > +endDate;
	const endTime = reversed ? +startDate : +endDate;
	let currentDate = reversed ? endDate : startDate;
...
function eachWeekendOfInterval(interval$1) {
	const dateInterval = eachDayOfInterval(interval$1);
	const weekends = [];
	let index = 0;
...
//#endregion
//#region node_modules/date-fns/locale/en-US/_lib/formatLong.mjs
var dateFormats = {
	full: "EEEE, MMMM do, y",
	long: "MMMM do, y",
	medium: "MMM d, y",
...
//#endregion
//#region node_modules/date-fns/locale/en-US/_lib/match.mjs
...
function formatDistance(date, baseDate, options) {
	...
	months$1 = differenceInMonths(dateRight, dateLeft);
	if (months$1 < 12) {
...
function formatDistanceStrict(date, baseDate, options) {
	...
	const dstNormalizedMinutes = (milliseconds$1 - (getTimezoneOffsetInMilliseconds(dateRight) - getTimezoneOffsetInMilliseconds(dateLeft))) / millisecondsInMinute;
...
function getDaysInMonth(date) {
	...
	const lastDayOfMonth$1 = constructFrom(date, 0);
...
function intervalToDuration(interval$1) {
	const start = toDate(interval$1.start);
	const end = toDate(interval$1.end);
	const duration = {};
...
function intlFormatDistance(date, baseDate, options) {
	...
	if (!options?.unit) {
		...
		if (Math.abs(diffInSeconds) < secondsInMinute) {
			...
			unit = "hour";
		} else if (Math.abs(diffInSeconds) < secondsInWeek && (value = differenceInCalendarDays(dateLeft, dateRight)) && Math.abs(value) < 7) unit = "day";
			...
			unit = "month";
		} else if (Math.abs(diffInSeconds) < secondsInYear) if (differenceInCalendarQuarters(dateLeft, dateRight) < 4) {
...
function isWithinInterval(date, interval$1) {
	...
	const [startTime, endTime] = [+toDate(interval$1.start), +toDate(interval$1.end)].sort((a, b) => a - b);
...
function sub(date, duration) {
	const { years = 0, months: months$1 = 0, weeks = 0, days: days$1 = 0, hours = 0, minutes = 0, seconds = 0 } = duration;
	const dateWithoutDays = subDays(subMonths(date, months$1 + years * 12), days$1 + weeks * 7);
