...
import { t as require_jsx_runtime } from "./jsx-runtime-Dsiv39fL.js";
import { t as require_react_dom } from "./react-dom-D2_PWQMy.js";
import { C as pick, E as invariant, S as last, a as joinPaths, c as parsePathname, d as resolvePath, f as trimPath, g as deepEqual, h as createControlledPromise, i as interpolatePath, m as trimPathRight, n as cleanPath, p as trimPathLeft, r as exactPathTest, s as matchPathname, t as rootRouteId, u as removeTrailingSlash, v as functionalUpdate, w as replaceEqualDeep, y as hasUriEncodedChars } from "./root-DaK8jZe-.js";
...
function createBrowserHistory(opts) {
	...
	if (!((_a = win.history.state) == null ? void 0 : _a.key)) win.history.replaceState({
		[stateIndexKey]: 0,
		key: createRandomKey()
	}, "");
...
function setupScrollRestoration(router, force) {
	if (scrollRestorationCache === void 0) return;
	if (force ?? router.options.scrollRestoration ?? false) router.isScrollRestoring = true;
	if (typeof document === "undefined" || router.isScrollRestorationSetup) return;
...
function decode(str, pfx) {
	...
	return [...new URLSearchParams(searchParamsPart).entries()].reduce((acc, [key, value]) => {
		const previousValue = acc[key];
		if (previousValue == null) acc[key] = toValue(value);
		else acc[key] = Array.isArray(previousValue) ? [...previousValue, toValue(value)] : [previousValue, toValue(value)];
		return acc;
	}, {});
}
	...
	*/
	constructor(options) {
		...
		this.getMatchedRoutes = (next, dest) => {
			...
			const getMatchedParams = (route) => {
				return matchPathname(this.basepath, trimmedPath, {
					to: route.fullPath,
		...
		this.buildLocation = (opts) => {
			const build = (dest = {}, matchedRoutesResult) => {
				...
				if (Object.keys(nextParams).length > 0) matchedRoutesResult?.matchedRoutes.map((route) => {
					var _a2;
			...
			const buildWithMatches = (dest = {}, maskedDest) => {
				...
				const final = build(dest, this.getMatchedRoutes(next, dest));
				if (maskedNext) {
					const maskedMatches = this.getMatchedRoutes(maskedNext, maskedDest);
					final.maskedLocation = build(maskedDest, maskedMatches);
				}
				return final;
			};
		...
		this.load = async (opts) => {
			...
			loadPromise = new Promise((resolve) => {
				this.startTransition(async () => {
					...
					if (this.latestLoadPromise === loadPromise) {
						(_a = this.commitLocationPromise) == null || _a.resolve();
			...
			if (this.hasNotFoundMatch()) this.__store.setState((s) => ({
				...s,
				statusCode: 404
			}));
		};
			...
			if (shouldViewTransition && typeof document !== "undefined" && "startViewTransition" in document && typeof document.startViewTransition === "function") {
				...
				if (typeof shouldViewTransition === "object" && this.isViewTransitionTypesSupported) {
					...
					startViewTransitionParams = {
						update: fn,
						types: typeof shouldViewTransition.types === "function" ? shouldViewTransition.types(getLocationChangeInfo({
							resolvedLocation: prevLocation,
							location: next
		...
		this.loadMatches = async ({ location, matches, preload: allPreload, onReady, updateMatch = this.updateMatch, sync }) => {
			...
			const handleRedirectAndNotFound = (match, err) => {
				...
				if (isRedirect(err) || isNotFound(err)) {
					...
					(_a = match.beforeLoadPromise) == null || _a.resolve();
					(_b = match.loaderPromise) == null || _b.resolve();
					(_c = match.loadPromise) == null || _c.resolve();
					if (isRedirect(err)) {
						rendered = true;
						...
						try {
							const handleSerialError = (index, err, routerCode) => {
								...
								updateMatch(matchId, (prev) => {
									var _a3, _b3;
									(_a3 = prev.beforeLoadPromise) == null || _a3.resolve();
									(_b3 = prev.loadPromise) == null || _b3.resolve();
									return {
										...prev,
										error: err,
										status: "error",
							...
							for (const [index, { id: matchId, routeId }] of matches.entries()) {
								...
								if (executeBeforeLoad) {
									...
									updateMatch(matchId, (prev) => {
										var _a2;
										(_a2 = prev.beforeLoadPromise) == null || _a2.resolve();
										return {
											...prev,
											beforeLoadPromise: void 0,
											isFetching: false
										...
										const runLoader = async () => {
											...
											try {
												...
												try {
													...
													try {
														(_d2 = (_c2 = route.options).onError) == null || _d2.call(_c2, e);
													} catch (onErrorError) {
														error = onErrorError;
												...
												(_e = this.serverSsr) == null || _e.onMatchSettled({
													router: this,
		...
		this._handleNotFound = (matches, err, { updateMatch = this.updateMatch } = {}) => {
			...
			for (const match of matches) matchesByRouteId[match.routeId] = match;
			if (!routeCursor.options.notFoundComponent && ((_a = this.options) == null ? void 0 : _a.defaultNotFoundComponent)) routeCursor.options.notFoundComponent = this.options.defaultNotFoundComponent;
		...
		if (typeof document !== "undefined") window.__TSR_ROUTER__ = this;
	}
	get state() {
	...
	matchRoutesInternal(next, opts) {
		...
		const getParentContext = (parentMatch) => {
			return !(parentMatch == null ? void 0 : parentMatch.id) ? this.options.context ?? {} : parentMatch.context ?? this.options.context ?? {};
		};
		...
		matches.forEach((match, index) => {
			...
			if (!this.getMatch(match.id) && (opts == null ? void 0 : opts._buildLocation) !== true) {
				const parentMatch = matches[index - 1];
...
var isMatch = (match, path) => {
	...
	let part;
	let value = match;
	while ((part = parts.shift()) != null && value != null) value = value[part];
	return value != null;
};
...
function useStore(store, selector = (d) => d) {
	return (0, import_with_selector.useSyncExternalStoreWithSelector)(store.subscribe, () => store.state, () => store.state, selector, shallow$1);
}
...
function useIntersectionObserver(ref, callback, intersectionObserverOptions = {}, options = {}) {
	const isIntersectionObserverAvailable = import_react.useRef(typeof IntersectionObserver === "function");
	const observerRef = import_react.useRef(null);
	import_react.useEffect(() => {
...
function Transitioner() {
	...
	if (!router.isServer) router.startTransition = (fn) => {
	...
	useLayoutEffect(() => {
		if (typeof window !== "undefined" && router.clientSsr || mountLoadForRouter.current.router === router && mountLoadForRouter.current.mounted) return;
...
function CatchNotFound(props) {
	...
	return /* @__PURE__ */ (0, import_jsx_runtime.jsx)(CatchBoundary, {
		...
		onCatch: (error, errorInfo) => {
			var _a;
			if (isNotFound(error)) (_a = props.onCatch) == null || _a.call(props, error, errorInfo);
			else throw error;
		},
...
var require_jsesc = /* @__PURE__ */ __commonJS({ "node_modules/jsesc/jsesc.js": ((exports, module) => {
	var object = {};
	...
	var jsesc$1 = (argument, options) => {
		...
		if (!isString(argument)) {
			if (isMap(argument)) {
				...
				return "new Map(" + jsesc$1(Array.from(argument), options) + ")";
			}
			if (isSet(argument)) {
				if (argument.size == 0) return "new Set()";
				return "new Set(" + jsesc$1(Array.from(argument), options) + ")";
			}
			if (isBuffer(argument)) {
				if (argument.length == 0) return "Buffer.from([])";
				return "Buffer.from(" + jsesc$1(Array.from(argument), options) + ")";
			}
			if (isArray(argument)) {
				result = [];
				options.wrap = true;
				...
				forEach(argument, (value) => {
					...
					result.push((compact || inline2 ? "" : indent) + jsesc$1(value, options));
				});
				if (isEmpty) return "[]";
				...
				forOwn(argument, (key, value) => {
					isEmpty = false;
					result.push((compact ? "" : indent) + jsesc$1(key, options) + ":" + (compact ? "" : " ") + jsesc$1(value, options));
				});
				if (isEmpty) return "{}";
				return "{" + newLine + result.join("," + newLine) + newLine + (compact ? "" : oldIndent) + "}";
			}
	...
	jsesc$1.version = "3.0.2";
	module.exports = jsesc$1;
}) });
...
function ScrollRestoration$1() {
	...
	return /* @__PURE__ */ (0, import_jsx_runtime.jsx)(ScriptOnce, {
...
var Match = import_react.memo(function MatchImpl({ matchId }) {
	...
	const ResolvedSuspenseBoundary = (!route.isRoot || route.options.wrapInSuspense) && (route.options.wrapInSuspense ?? PendingComponent ?? ((_b = route.options.errorComponent) == null ? void 0 : _b.preload)) ? import_react.Suspense : SafeFragment;
	...
	return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)(import_jsx_runtime.Fragment, { children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)(matchContext.Provider, {
		value: matchId,
...
var MatchInner = import_react.memo(function MatchInnerImpl({ matchId }) {
	...
	if (match.status === "error") {
		if (router.isServer) return /* @__PURE__ */ (0, import_jsx_runtime.jsx)(RouteErrorComponent, {
			error: match.error,
			reset: void 0,
			info: { componentStack: "" }
		});
...
function Matches() {
	const router = useRouter();
	const pendingElement = router.options.defaultPendingComponent ? /* @__PURE__ */ (0, import_jsx_runtime.jsx)(router.options.defaultPendingComponent, {}) : null;
	const inner = /* @__PURE__ */ (0, import_jsx_runtime.jsxs)(router.isServer || typeof document !== "undefined" && router.clientSsr ? SafeFragment : import_react.Suspense, {
		fallback: pendingElement,
...
function MatchesInner() {
	...
	return /* @__PURE__ */ (0, import_jsx_runtime.jsx)(matchContext.Provider, {
		value: matchId,
		children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)(CatchBoundary, {
			getResetKey: () => resetKey,
			...
			children: matchId ? /* @__PURE__ */ (0, import_jsx_runtime.jsx)(Match, { matchId }) : null
		})
	});
}
...
function useLinkProps(options, forwardedRef) {
	...
	const next = import_react.useMemo(() => router.buildLocation(_options), [
		router,
		_options,
		currentSearch
	]);
	const preload = import_react.useMemo(() => {
		if (_options.reloadDocument) return false;
	...
	}, [doPreload]), { rootMargin: "100px" }, { disabled: !!disabled || !(preload === "viewport") });
	useLayoutEffect(() => {
...
function lazyRouteComponent(importer, exportName, ssr) {
	...
	const lazyComp = function Lazy(props) {
		...
		if (!comp) throw load();
		if ((ssr == null ? void 0 : ssr()) === false) return /* @__PURE__ */ (0, import_jsx_runtime.jsx)(ClientOnly, {
			fallback: /* @__PURE__ */ (0, import_jsx_runtime.jsx)(Outlet, {}),
			children: import_react.createElement(comp, props)
		});
...
function Asset({ tag, attrs, children }) {
	switch (tag) {
		...
		case "script":
			if (attrs && attrs.src) return /* @__PURE__ */ (0, import_jsx_runtime.jsx)("script", {
				...attrs,
				suppressHydrationWarning: true
			});
...
function HeadContent() {
	return useTags().map((tag) => /* @__PURE__ */ (0, import_react.createElement)(Asset, {
		...tag,
...
//#endregion
export { useLoaderDeps as $, getRouteApi as A, isNotFound as At, useMatches as B, RootRoute as C, redirect as Ct, createRootRouteWithContext as D, stringifySearchWith as Dt, createRootRoute as E, parseSearchWith as Et, useLinkProps as F, createMemoryHistory as Ft, CatchNotFound as G, Match as H, MatchRoute as I, tiny_warning_esm_default as It, useStableCallback as J, DefaultGlobalNotFound as K, Matches as L, Link as M, createBrowserHistory as Mt, createLink as N, createHashHistory as Nt, createRoute as O, decode as Ot, linkOptions as P, createHistory as Pt, useParams as Q, useChildMatches as R, NotFoundRoute as S, isRedirect as St, RouteApi as T, defaultStringifySearch as Tt, Outlet as U, useParentMatches as V, ScriptOnce as W, useNavigate as X, Navigate as Y, useSearch as Z, FileRouteLoader as _, SearchParamError as _t, useLocation as a, getRouterContext as at, createLazyFileRoute as b, getInitialRouterState as bt, useBlocker as c, ErrorComponent as ct, RouterContextProvider as d, retainSearchParams as dt, useLoaderData as et, RouterProvider as f, stripSearchParams as ft, FileRoute as g, PathParamError as gt, lazyRouteComponent as h, defer as ht, useCanGoBack as i, useRouter as it, rootRouteWithContext as j, notFound as jt, createRouteMask as k, encode as kt, ScrollRestoration as l, Await as lt, createRouter as m, TSR_DEFERRED_PROMISE as mt, HeadContent as n, matchContext as nt, useRouteContext as o, ClientOnly as ot, Router as p, isMatch as pt, useLayoutEffect as q, Asset as r, useRouterState as rt, Block as s, CatchBoundary as st, Scripts as t, useMatch as tt, useElementScrollRestoration as u, useAwaited as ut, LazyRoute as v, componentTypes as vt, Route as w, defaultParseSearch as wt, createLazyRoute as x, lazyFn as xt, createFileRoute as y, defaultSerializeError as yt, useMatchRoute as z };
//# sourceMappingURL=esm-DT8SKlTZ.js.map